...
const BACKOFF_MAX_MS = Number(process.env.TWITTER_BACKOFF_MAX_MS ?? 15 * 60 * 1000)

type TweetQueueItem = { event: any; attempts: number }
const tweetQueue: TweetQueueItem[] = []
let isProcessing = false
let pauseUntilMs = 0
let twitterClient: (TwitterApi & TwitterApiReadWrite) | TwitterApiReadWrite | undefined
...
const keyForQueueItem = (item: TweetQueueItem): string => {
  if (item?.event?.kind === 'sweep') {
    const tx = item?.event?.txHash ?? txHashFor(item?.event?.events?.[0]) ?? 'unknown'
    return `sweep:${tx}`
  }
  ...
  if (event?.kind === 'sweep' && Array.isArray(event?.events)) {
    ...
    for (const imageUrl of images) {
      try {
        ...
        console.error(`${logStart}Twitter - Sweep media upload failed; continuing:`)
    ...
    const params: any = media_ids.length > 0 ? { text, media: { media_ids } } : { text }
  ...
  try {
    ...
    while (tweetQueue.length > 0) {
      ...
      if (pauseUntilMs > now) {
        ...
        console.log(`${logStart}Twitter - Paused until reset. Waiting ${Math.ceil(waitMs / 1000)}s…`)
        await timeout(waitMs)
        pauseUntilMs = 0
        if (dailyLimitActive) {
          const maxAfterReset = 5
          const snapshot = dailyLimitSnapshotKeys ?? tweetQueue.map((i) => keyForQueueItem(i))
      ...
      try {
        ...
        console.error(`${logStart}Twitter - Unrecoverable error, dropping event:`)
        console.error(error)
        tweetQueue.shift()
      }
    }
...
export const tweetEvents = async (events: any[]) => {
  ...
  for (const [hash, evts] of hashToEvents.entries()) {
    if (evts.length > 4) {
      ...
      tweetQueue.push({ event: { kind: 'sweep', txHash: hash, events: evts }, attempts: 0 })
    }
  }
